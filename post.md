...
- Is there anything better in UEFI payload over SeaBIOS/GRUB ?
...
Any my talk on ECC 2017 [Booting UEFI-aware OS on coreboot enabled platform - "In God's Name, Why?"](https://www.youtube.com/watch?v=nt0BkqVUu3w&feature=youtu.be)

All my modifications of edk2 for the article below can be found in [3mdeb edk2 fork](https://github.com/3mdeb/edk2/tree/apu2-uefi-dbg)
...
```shell
git clone https://github.com/3mdeb/edk2.git -b apu2-uefi-dbg
```

To get rid of system dependencies, which can be problematic when building edk2
please use out container:
...
After that I was able to build `UEFIPAYLOAD.fd`:
...
Copy resulted payload file to coreboot directory e.g.:

```
cp Build/CorebootPayloadPkgIA32/DEBUG_GCC5/FV/UEFIPAYLOAD.fd ../coreboot
```

### Short path to compile UEFI Shell for apu2

For those who don't care about details procedure for building working image
looks like that.

Pull `coreboot-sdk` container:

```shell
docker pull coreboot/coreboot-sdk:1.50
```

Get PC Engines coreboot source tree:

```shell
git clone https://review.coreboot.org/coreboot.git
git remote add pcegnies https://github.com/pcengines/coreboot.git 
git fetch pcengines
git chekcout coreboot-4.6.x-uefi
```

Run it with mounted coreboot source tree:

```shell
docker run --rm -it -w /home/coreboot/coreboot \
...
```shell
make distclean
cp configs/pcengines_apu2_uefi.config .config
make
```

Now you can flash image (`build/coreboot.rom`) on your board:

```
flashrom -p internal -w /tmp/coreboot.rom
```

## TL;DR Hacking UEFI payload to boot on apu2

Below I noted things that I had to fix to boot UEFI Shell on apu2.
