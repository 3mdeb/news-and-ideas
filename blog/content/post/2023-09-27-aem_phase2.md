...
external storage. In our case the boot partition is `/dev/sda1` (yours may be
...
Note the PCR values with `sudo tpm2_pcrread sha256 > drtm_pcrs.txt` for
example and reboot the machine. If everything went correctly, the PCRs should
not change after reboot (you can comapre by reading the PCRs again with
`tpm2_pcrread` to a file and `diff` command when booted) and you should be
presented a TOTP code during LUKS key file password prompt, like this:
