...
Assuming that the RPMs and ZIP are in your Dom0 home directory already, open
...
demonstration purposes. So in your case feel free to install AEM on an
external storage, or omit `-m` to install it without MFA. In our case the boot partition is `/dev/sda1` (your may be
...
correctly, the PCRs should not change after reboot (you can compare by reading
...
safety timer. If your machine was not safely shut down (e.g. due to a power loss) and it had been running for less than approximately 70 minutes
(2^22 ms) since, you may experience such issue. Simply try to reboot the machine at
