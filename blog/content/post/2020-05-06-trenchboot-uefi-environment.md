...
install tools from official packages or build your ones. The second feature
will be strongly utilized in the entire project.
...
(supported and/or compatible) configuration, we can't provide full reliability.
...
1. Download a minimal installer image from the [website](https://nixos.org/nixos/download.html).

2. Flash USB drive with the image.

3. Insert the USB drive on the target platform and choose the proper boot
   option.
...
9. Generate initial configuration:
    ...
    >Above command will create `configuration.nix` file. It contains all
    default configuration options according to which NixOS will be installed.
...
8. Clone [3mdeb/nixos-trenchboot-configs](https://github.com/3mdeb/nixos-trenchboot-configs.git)
   repository.
    ...
    Let's take a closer look at its content. The entire file is rather large,
    so the output will be truncated and only essential parts/lines will be
...
9. Copy all configuration files to `/etc/nixos/` directory and replace
   the configuration for EFI.
...
section, I will show you, how to verify each component of the system to make
you sure about its correctness. Also, I will present how we met the second
stage project's requirements.
...
1. Reboot platform. In GRUB menu choose `"NixOS - Default"` entry (without
   DRTM).
    ...
    Collect logs during boot to be able to verify them. Using `dmesg` command
    in NixOS doesn't work because it doesn't show pre-kernel stage logs! The
    correct boot log is shown below.
    ...
    **Verification**: As expected, the bootloader executes Linux kernel
    directly. Platform booted without DRTM then.

2. Reboot platform once again. In GRUB menu choose `"NixOS - Secure Launch"`
   entry.

    Once again, collect logs during boot to be able to verify them. Using
    `dmesg` command in NixOS doesn't work, as in the previous case. The correct
...
1. If not already booted to `"NixOS - Secure Launch"`, reboot platform, and
   boot to NixOS via `"NixOS - Secure Launch"` entry in GRUB menu.
...
3. Run `extend_all.sh` script from `landing-zone` package.
    ...
    `/nix/store/3w98shnz1a6nxpqn2wwn728mr12dy3kz-linux-5.5.3/bzImage`
    is directory to Linux kernel.
...
5. Go to `/nix/store/lf763br9hm0ipp76k2p16iq75x3xpgrm-landing-zone-0.3.0`
   directory.
...
6. Execute `./extend_all.sh` script.
    ...
    `tpm2_pcrread` output. If DRTM is enabled and executes properly, they
    should be the same. It proves that the LZ code utilizes the SHA256 algorithm during
    measurements.
