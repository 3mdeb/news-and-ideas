...
The components within the Secure Enclave subsystem: a Random Number Generator,
which creates entropy. An internal AES engine -- so there is an internal crypto
engine which uses UID and GID fuses, which are never exposed. There is a Public
Key Accelerator, PKA, for RSA and ECC operations. And of course the Secure
Enclave Processor, the dedicated CPU that runs sepOS, which is an L4-based
microkernel. Finally, the Memory Protection Engine, which encrypts all
communication between the Secure Enclave and external memory with ephemeral
keys generated at each boot.
...
with improved security. This is interesting -- Apple probably detected something
...
receiving sepOS -- it provides coprocessor isolation. When SCIP is configured
...
memory region, and the kernel becomes read-only after boot -- no runtime
...
The security properties: no writable kernel mappings -- even the kernel can't
write to its own code. No executable code outside the protected region -- you
can't inject code elsewhere. Configuration is locked at boot, and the attacker
cannot reconfigure that.

The x86 comparison: memory controller lock is similar to UEFI DXE image
protection -- both are set at the bootloader stage. There is a signal in UEFI
-- `EndOfDxe` and `SmmReadyToLock` -- sent at the end of the configuration
phase. After this, image sections marked as code become read-only. The
opportunity: make this mandatory, not optional, across all implementations.
...
proceeds. The innovation could be: this firmware update mode through the Dasharo
Tools Suite or maybe even capsule update -- it would warn the user if the PCR
...
Take it further: a BIOS setup screen showing current PCR values, where the user
...
was created and the result tells them -- okay, this set of PCRs is good, is
...
backend introspection -- what's going on in my fleet of devices, what's their
...
The solution: before update, compute expected PCR values for the new firmware
...
systems like VPN, maybe some VMs which contain some special data -- they could
be gated.
...
We see that from the L4 microkernel for sepOS -- minimal size, very high
...
to get to the buses and possibly probe -- extremely expensive equipment needed
