---
title: A new source of trust for your platform - Intel TXT for Dasharo
...
Dell OptiPlex 7010 / 9010 is Intel TXT capable. All you need is a Intel TXT
...
to start with the firmware, the inevitable piece of code initializing the
silicon and other hardware. Its responsibility is to prepare the platform to
run operating system, but also to load it. So basically the owner of the
firmware has control what operating system can be launched, and what is most
important, how it can be launched. By how we should understand whether the
operating system is launched as intended without any undesired actions, like
malware or spyware installation under the hood. In a case the attacker has
malformed the firmware on your platform he may install key loggers and spying
software to steal your data, passwords, etc. How to prevent that? How to ensure
the operating system software was not tampered with? The answer is DRTM (
Dynamic Root of Trust for Measurement). It allows to securely measure the
operating system (calculate hash of its software components) in an unreplayable
manner and store it inside a secure storage chip, i.e. the Trusted Platform
Module.

## Intel TXT deep dive

Below diagram represents the typical boot flow of Intel TXT enabled machine
with measured launch.
...
to initialize the Intel TXT to be used by operating system bootloader. For this
purpose it calls the BIOS ACM (Authenticated Code Module) to properly set up
Intel TXT. It must be noted that ACMs are signed by Intel and Intel CPU will
only execute ACMs signed by Intel key. The key hash is burned into CPUs an is
used to authenticate microcode updates and ACM signatures. Such measures ensure
only the authorized code is able to initialize Intel TXT, which prevents its
...
The above diagram represents the tboot case. Tboot is responsible here for
...
case). If all checks pass tboot loads an SINIT ACM, specifies the memory ranges
...
ACM calculate the hashes of the operating system software and sends the mot the
...
but how it is different in case of TXT?

1. There are 24 PCRs inside TPM in total. BIOS can only extend PCRs 0 to 16 and
   PCR 23. PCRs 17 to 22 can be only enabled by hardware, no software code is
   able to initialize them, e.g. the microcode when launching the Intel TXT.
   Before the PCR 17 to 22 are initialized their values are set to FFs (-1).
   This is achieved by Intel TXT opening the TPM's locality 4, the only
   locality able to reset the PCRs 17-22. Thsi locality is inaccessible by
   software.
2. SINIT ACM is being measured into PCR 17 as soon as TPM enters locality 4 via
   GETSEC\[SENTER\] instruction execution. There is no possibility to alter
   this.
...
(SMIs) in order to provide a safe environment for the operating system software
...
which measurement isn't approved. It is called a Launch Control Policy a user
generated structure which defines various requirements to be fulfilled to
consider the software/platform trusted, e.g. define the hashes of the operating
system software that are allowed to boot.
...
- Incorrectly checked information about production or debug chipset:
...
fix any wrong assumptions about the requirements for calling an ACM. When
...
Simple as it may sound, but in fact it wasn't. My first attempts of enabling
Intel TXT were over a year ago which were also presented on
[QubesOS and 3mdeb minisummit](https://www.youtube.com/watch?v=YE2FbFlszI4).

In order to use the Intel TXT you will need the BIOS ACM and the SINIT ACM. THe
...
In case you have access to both here is a short guide how to configure coreboot
to use Intel TXT:
...
using the guide from repository. Here is an example of running Anti Evil Maid tboot
to boot QubesOS:

[![asciicast](https://asciinema.org/a/449387.svg)](https://asciinema.org/a/449387?speed=1)

Another interesting case in remote attestation of the operating system
software. However there are not many ready to go solutions which could leverage
...
attestation USB token which will free the user from using a doubtful
attestation servers and the global network by implementing the attestation
services based on CHARRA inside a USB dongle. For more information please visit
the [Fobnail page](https://fobnail.3mdeb.com/).
...
leverages the Intel TXT and AMD Secure Startup for this purpose. At the day of
...
firmware Dell OptiPlex 7010 / 9010. It will be fully tested with an automated
...
where you can find the most recent information about Dasharo ecosystem and
discussions about open source firmware. Join the community of hardware and open
source firmware enthusiasts today!
