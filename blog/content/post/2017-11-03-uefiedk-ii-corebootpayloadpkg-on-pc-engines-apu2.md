...
platform. For interested ones in that topic I recommend to take look at
[video from coreboot conference 2016][2]. All my modifications of edk2 for the
article below can be found in [3mdeb edk2 fork][3] For those interested in
UEFI-aware OS booting this blog post can be useful, but I also plan to write
something straightforward that can be used and read by APUx platform users.
...
On my Debian testing I had to explicitly change compilers to `gcc-5` and
`g++-5`:
...
`Build/CorebootPayloadPkgIA32/DEBUG_GCC5/FV/UEFIPAYLOAD.fd`. Following
[build and integration instructions][5] I added build result as
...
`Valid RAM and Time` (`VRT`) bit in RTC Date Alarm register (aka Register D). By
