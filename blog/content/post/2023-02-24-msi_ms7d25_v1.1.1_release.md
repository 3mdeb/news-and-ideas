...
The past three months were mostly spent on debugging and resolving issues reported
...
### [MSI PRO Z690-A WIFI DDR4 doesn't initialize IGP on certain Processors models](https://github.com/Dasharo/dasharo-issues/issues/274)
  ...
  give firmware output on the screen. If you are unable to boot the DTS and do
  Dasharo HCL in such situations, then try dumping as much information as you
  can if you manage to boot a Linus OS using the [following guides](https://docs.dasharo.com/common-coreboot-docs/dumping_logs/)
...
### [Intel XTU on Windows reports "The platform does not support overclocking" on the MSI PRO Z690-A WIFI DDR4 with a K-series Processor](https://github.com/Dasharo/dasharo-issues/issues/159)
  ...
  causing the CPU frequency to be lower than the maximum default value (and so
...
[Dasahro Security Options](https://docs.dasharo.com/dasharo-menu-docs/dasharo-system-features/).
Both `BIOS boot medium lock` and `Enable SMM BIOS write protection` should be
unchecked. [UEFI Secure boot](https://docs.dasharo.com/dasharo-menu-docs/device-manager/#secure-boot-configuration)
...
You can use flashrom from DTS Shell or your own Linux OS with up to date
flashrom for firmware update purpose. Procedure for getting the right flashrom
is described in Build flashrom section in the
[Initial deployment documentation](https://docs.dasharo.com/variants/msi_z690/initial-deployment/#initial-deployment-manually)

You will need to download a firmware update binary. When you boot DTS enter the
shell from the menu and invoke following commands (they will also work in any
Linux terminal with wget installed):
...
If you already had Dasharo v1.1.0, simply follow the procedures explained in
the [documentation for v1.1.0 or newer releases](https://docs.dasharo.com/variants/msi_z690/firmware-update/#version-v110-or-newer).

If you had original MSI's firmware or Dasharo v1.0.0 on the DDR4 platform
follow the [documentation for release older than v1.1.0](https://docs.dasharo.com/variants/msi_z690/firmware-update/#version-older-than-v110).

**IMPORTANT**: Note you will need to reboot the machine afterward. If you are
migrating from MSI's original firmware, you will also notice a blinking SATA
LED which will indicate that you need to press the power button for 5s to shut
down the machine and then power it up again (this process will unlock the
memory controller registers and let Dasharo boot). The process with power
button is not needed if you update from previous Dasharo.

If the platform booted successfully, you may enable the protections you had
earlier back.
...
on March 16th at 5 PM UTC! Our main goal is to build a strong community around
Dasharo and open-source enthusiasts, and we want you to be a part of it. During
the call, you'll have the opportunity to connect with other community members,
ask questions, and provide feedback on our current activities. You'll also be
the first to hear about exciting updates and new developments.
