...
* Black dashed arrow: getting platform to power off state with breaking
...
(check the red dashed arrows): from platform power off state with previous
system not properly shut down or without powering off platform.
...
by changing [Dasharo firmware](https://www.dasharo.com/) boot order. But the
application was booted by chosing the needed drive in `One Time Boot` menu. When
bootign the application by manually choosing the hard drive it **is important to
make exaclty the same steps every time**, that is, making the same amount of
keyboard klicks and visiting the same pages. Otherwise firmware will generate
another set of data in RAM and make the second step from above useless.
...
one cold boot attack utilizing Qemu Monitor `system_reset` functionality.
...
The test setup using following keys for LUKS:
...
> **Important note**: this security information leak was done by professionals
> on purpose, do not share your secrets with anyone!

## ram-wipe tests

### Running first stage only

To make `ram-wipe` run first stage only on reboot the Linux kernel command line
parameter `wiperamexit` was set to `no`. This resulted in `kexec` fail because
of `ram-wipe-kexec-prepare.service` not starting, hence the second Linux kernel
was not booted and the second `ram-wipe` stage not running:
...
With the following pages and adresses excluded as used by firmware (inf. from
the EFI application):
...
`ram-wipe` is an application and cannot ovewrite kernel space addresses.
Analysis of other addresses:

* `0x1000-0x10000`: not zeroed completely, but `hexdump` showed, that the only
  ...
  bootloader.
* `0x100000-0x800000`: 524288 bytes zeroed and 6815744 not zeroed. All not
  zeroed bytes contain [the
  pattern](https://github.com/zarhus/ram-dump-efi/blob/361b3637218c72f5b3b23e09455df63daafffb84/app.c#L5)
  written by the EFI application, hence, these bytes were not used by Linux or bootloader.
* `0x1600000-0x75ff0000`: 2026242048 zeroed and 28049408 not zeroed. Most of not
  zeroed addresses contain some data, others contain [the
  pattern](https://github.com/zarhus/ram-dump-efi/blob/361b3637218c72f5b3b23e09455df63daafffb84/app.c#L5).
* `0x7bd48000-0x7d922000`: 26517504 bytes zeroed and 2686976 not zeroed. Not
  zeroed bytes contain Grub executable and its modules.
* `0x7f5ee000-0x7f5f4000`: completely zeroed.
* `0x7fc00000-0x7fc70000`: completely zeroed.
* `0x7fd20000-0x7fd2d000`: completely zeroed.
* `0x100000000-0x180000000`: this region is very fragmentated in terms of zeroed
  and not zeroed addresses. 1607467008 bytes was zeroed and 540016640 was not
  zeroed. With 36451452 bytes taken by the kernel and 503565188 used by
  processes.

After raw anlysis it is a high time for some real attack. The attack will be
...
So endeed the first `ram-wipe` staged saved the keys.
...
Hence these ranges should be excluded from analysis, because `sdmem` used in
`ram-wipe` is an application and cannot ovewrite kirnel space addresses.
Analysis of other addresses:
...
* `0x1000-0x10000`: not zeroed completely, but `hexdump` showed, that the only
  ...
  bootloader.
* `0x100000-0x800000`: 524288 bytes zeroed and 6815744 not zeroed. All not
  zeroed bytes contain [the pattern](https://github.com/zarhus/ram-dump-efi/blob/361b3637218c72f5b3b23e09455df63daafffb84/app.c#L5)
  written by the EFI application, hence, these bytes were not used by Linux or
  bootloader.
* `0x1600000-0x75ff0000`: 71892992 bytes zeroed and 1982398464 bytes not zeroed.
  Most of not zeroed addresses contain mostly Grub data, others contain [the
  pattern.](https://github.com/zarhus/ram-dump-efi/blob/361b3637218c72f5b3b23e09455df63daafffb84/app.c#L5)
* `0x7bd48000-0x7d922000`: completely not zeroed, contains process data.
* `0x7f5ee000-0x7f5f4000`: completely not zeroed, contains process data.
* `0x7fc00000-0x7fc70000`: completely not zeroed, contains process data.
* `0x7fd20000-0x7fd2d000`: completely not zeroed, contains process data.
* `0x100000000-0x180000000`: this region is very fragmentated in terms of zeroed
...
Both keys were successfully stolen after cold boot! Hence running only first
`ram-wipe` stage does not protect from cold boot.
...
* `0x100000-0x800000`: 524288 bytes zeroed and 6815744 not zeroed. All not
  ...
  written by the EFI application, hence, these bytes were not used by Linux or
  bootloader.
* `0x1600000-0x7bd20000`: 2047803392 zeroed and 6488064 not zeroed. Some of not
  zeroed addresses contain some data, others contain [the
  pattern](https://github.com/zarhus/ram-dump-efi/blob/361b3637218c72f5b3b23e09455df63daafffb84/app.c#L5).
* `0x7bd48000-0x7d922000`: completely zeroed.
* `0x7f5ee000-0x7f5f4000`: completely zeroed.
* `0x7fc00000-0x7fc70000`: completely zeroed.
* `0x7fd20000-0x7fd2d000`: completely zeroed.
* `0x100000000-0x180000000`: this region is very fragmentated in terms of zeroed
  and not zeroed addresses. 1875509248 bytes was zeroed and 271974400 was not
  zeroed. The situation is the same ass before: a part of the not zeroed
  addresses was sued by the kernel and another part by processes.
...
* `0x100000-0x800000`: 524288 bytes zeroed and 6815744 not zeroed. All not
  ...
  written by the EFI application, hence, these bytes were not used by Linux or
  bootloader.
* `0x1600000-0x7bd20000`: 84869120 zeroed and 1969422336 not zeroed. Some of not
  zeroed addresses contain some data, others contain [the
  pattern](https://github.com/zarhus/ram-dump-efi/blob/361b3637218c72f5b3b23e09455df63daafffb84/app.c#L5).
* `0x7bd48000-0x7d922000`: completely not zeroed, contains data.
* `0x7f5ee000-0x7f5f4000`: completely not zeroed, contains data.
* `0x7fc00000-0x7fc70000`: completely not zeroed, contains data.
* `0x7fd20000-0x7fd2d000`: completely not zeroed, contains data.
* `0x100000000-0x180000000`: this region is very fragmentated in terms of zeroed
  and not zeroed addresses. 350945280 bytes was zeroed and 1796538368 was not
  zeroed. Still a lot of processes data not zeroed.

The attack on LUKS is still successfull:
...
stability and predictability saves this blogpost from another copy of logs from
the EFI application, because the memory map and excluded pages used by firmware
are the same as the one showed in previous sections and are the same for two
attacks described in the next two sections.
...
* `0x1600000-0x7bd20000`: 2054160384 zeroed and 131072 not zeroed. Some of not
  zeroed addresses contain some data, others contain [the
  pattern](https://github.com/zarhus/ram-dump-efi/blob/361b3637218c72f5b3b23e09455df63daafffb84/app.c#L5).
* `0x7bd48000-0x7d8d7000`: completely zeroed.
* `0x7f5ee000-0x7f5f4000`: completely zeroed.
* `0x7fc00000-0x7fc70000`: completely zeroed.
* `0x7fd20000-0x7fd2d000`: completely zeroed.
* `0x100000000-0x180000000`: this region is very fragmentated in terms of zeroed
  and not zeroed addresses. 1892876288 bytes was zeroed and 254607360 was not
  zeroed. Again the situation is the same ass before: a part of the not zeroed
  addresses was sued by the kernel and another part by processes.

And the attack shows positive results:
...
* `0x100000-0x800000`: 524288 bytes zeroed and 6815744 not zeroed. All not
  ...
  written by the EFI application, hence, these bytes were not used by Linux or
  bootloader.
* `0x1600000-0x7bd20000`: 84606976 zeroed and 1969684480 not zeroed. Most of not
  zeroed addresses contain some data.
* `0x7bd48000-0x7d922000`: completely not zeroed, contains data.
* `0x7f5ee000-0x7f5f4000`: completely not zeroed, contains data.
* `0x7fc00000-0x7fc70000`: completely not zeroed, contains data.
* `0x7fd20000-0x7fd2d000`: completely not zeroed, contains data.
...
* `ram-wipe` cannot wipe RAM used by kernel, because it is an application not
  allowed to modify Linux kernel address space. Hence if your secrets are being
  stored inside kernel address space, the `ram-wipe` will not help protecting
  them from warm boot attack.
* `ram-wipe` wipes memory previously used by some processes, but it cannot wipe
  ...
  sure** that `ram-wipe` **could and will** wipe RAM, where your proccess left
  your secrets. This is the reason to show difference of zeroed and not zeroed
...
What about cold boot attacks then? The tests showed, that `ram-wipe` does not
wipe anything during cold boot attack (the zeroes at some addresses in RAM are
caused by some processes zeroing out memory). The reason of this is conceptual:
as descussed above, `ram-wipe` relies on proper system reboot/shutdown/halt
...
* Make `ram-wipe` wipe RAM on boot too, this will make sure, that Linux
  userspace proccesses will not get access to secrets leftovers in RAM. This
  will need the Secure Boot to be inforced up to Linux kernel and RAM disk
  though, because there will be no point of wiping during Linux boot if an
  attacker will boot malware instead of needed kernel.
* Wipe RAM earlier in boot sequence, for example make firmware wippep RAM for
  you. This will make sure RAM is being wiped as soon as possible during boot
  and that the program that is responcible for tshi is fully trusted and have
  a very small TCB.

Thank you for reading this blogpost, and see you again soon!
