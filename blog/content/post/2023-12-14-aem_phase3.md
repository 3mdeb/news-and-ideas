...
Launch entry point across all supported architectures. Latest version of
specification can be found on [TrenchBoot
site](https://trenchboot.org/specifications/).
...
takes majority of specification. The SLRT is constructed from a header at the
beginning, followed by a list of Tag-Length-Value (TLV) entries.

Entries specify things like location of DCE (Dynamic Configuration Environment,
SINIT ACM in case of Intel TXT), DLME (Dynamic Launch Measured Environment, Xen
in our case) and TPM DRTM event log. Another very important entry called DRTM
Measurements Policy describes what components have to be measured by DLME, in
what order, to which PCR and where they are located in the memory. The size of
this entry may vary to provide flexibility for different number of components.
This allowed us to properly measure more than just two of modules passed to Xen
(dom0 kernel and initramfs), so previous limitation that excluded microcode and
...
and you're good to go. Just remember to use binaries for v0.3.0, which can be
downloaded from **TBD**.

If you're updating from previous AEM version, keep in mind that unsealing will
fail on the first boot. This is expected, and is described in AEM README
**TBD: add link after https://github.com/TrenchBoot/qubes-antievilmaid/pull/7
gets merged**.
...
Digests from the entries can be extended on debug PCR 16 (make sure it is reset
before starting!) and compared with actual values of PCR 17 and 18.
...
younger than 15 years should do, as long as the platform has TPM.

## How you can help

By testing, of course! Install AEM, see if it performs as it should and report
any problem, either directly on appropriate repository or in
[trenchboot-issues](https://github.com/TrenchBoot/trenchboot-issues) if it spans
multiple components or you're not sure which one should be used.
