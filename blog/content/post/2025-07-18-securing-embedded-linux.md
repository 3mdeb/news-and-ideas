...
- Creating UKI EFI image which allows us to boot directly from UEFI BIOS
...
add support for UEFI Secure Boot (SB). With UKI file we only need to sign and
...
depends on used `bootloader` and [Wic
plugin](https://docs.yoctoproject.org/dev-manual/wic.html) which prepares boot
partition. In case of no separate bootloader and `bootimg-partition` Wic plugin
we only need to add to image recipe:
...
script which should run before anything else requiring filesystem access.
Commands below are used to encrypt rootfs partition without losing data. Those
steps are done in `encrypt_device` function but to make them easier to follow I
replaced variables with values and removed checks and redirections:
...
to systemd service that starts when we boot with UEFI Secure Boot enabled.
...
A/B update allows us to do reliable updates that won't brick our device even if
update fails.
...
update our system. To use it we need to build cpio archive in correct format:
...
If anyone is interested in asking questions then you can join [Zarhus Developers
