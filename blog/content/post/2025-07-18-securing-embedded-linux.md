...
In this post I'll describe a couple of features implemented or planned to be
implemented in [meta-zarhus](https://github.com/zarhus/meta-zarhus) related to
system security. Some of those features are:

- Creating UKI EFI image which allows us to boot directly from BIOS
- Adding security modules and configuration to Linux kernel (Lockdown, module
  signing)
- Rootfs encryption and decryption with TPM
- A/B updates

Some of those features are located in separate sublayers in
[meta-zarhus-features](https://github.com/zarhus/meta-zarhus/blob/develop/meta-zarhus-features)
while others e.g. UKI generation is defined in x86-64 BSP sublayer.
Changes needed for combination of different features are contained in dynamic
layer that's inside feature sublayer.

## Unified Kernel Image (UKI)

When we started adding support for x86-64 platform we decided to skip bootloader
and build image with only one
[UKI](https://uapi-group.org/specifications/specs/unified_kernel_image/) file
which can be booted directly by BIOS. This decision was predicated on needing to
add support for UEFI Secure Boot. With UKI file we only need to sign and verify
that one file, which lowers attack surface which could be used to get around
Secure Boot, e.g. if we used GRUB as bootloader then we would also need to add
`shim` and sign and verify every file used by GRUB including `grub.cfg` which
unverifed could be used for
[BootHole](https://eclypsium.com/blog/theres-a-hole-in-the-boot/) attack

As of now file structure on boot (EFI) partition looks like this:
...
Where `bootx64.efi` is our UKI file. It's located under default path which is
searched by BIOS for boot files.

### Generating UKI

UKI files can be generated with `ukify` command. In Yocto we can either write
our own task which could look like this:
...
Generating UKI file in Yocto boils down to one `ukify build` command and correct
task preparation (adding dependencies and using correct variables). UKI file
generated this way will contain:
...
After generating this file we need to add it to boot partition. How we'll do it
depends on used `bootloader` and Wic plugin which prepares boot partition. In
case of no separate bootloader and `bootimg-partition` Wic plugin we only need
to add:
...
We can also generate our file by using already written BitBake class.
There is `uki.bbclass` in [poky
layer](https://git.yoctoproject.org/poky/tree/meta/classes-recipe/uki.bbclass?h=walnascar)
but only in `Walnascar` and newer versions. I had to copy this file to
`meta-zarhus` layer as it's using `Scarthgap` layer versions. I also had to add
couple small fixes to make sure it builds:
...
After that using this class if very easy, just add to your image recipe:
...
Of course this class can be configured much more, if you want to know more it's
best to read comments and check which variables use `?=` operator.

## System hardening

System hardening adds a couple of important changes related to securing our
...
- Enable kernel lockdown security module and run in most secure mode by default.
...
Verifying those changes is quite easy. To verify kernel lockdown we can check
`dmesg` logs and/or try to get access that's disallowed by lockdown e.g. by
trying to read memory via `/dev/mem`:
...
- Check which modules ale loaded by default after system start and choose one,
  e.g.
...
- Remove module signature. I had to mount Zarhus rootfs partition on host (tests
  were done in QEMU) as Zarhus doesn't include tools needed for this test:

  ```sh
  strip -g mnt/usr/lib/modules/6.6.21-yocto-standard/kernel/net/8021q/8021q.ko
  ```

  Before removing signatures `modinfo` returned this output
  ...
  After removing signatures all rows after `vermagic` weren't included.

- Boot into Zarhus OS again and check if module loaded successfully and if
  `dmesg` informs us of failed signature verification:
...
Next feature that was implemented was rootfs encryption. High-level boot flow
looks like this:
...
In reality there is much more happening under the hood. Zarhus OS initramfs uses
...
Adding our own script which will be started by initramfs is as easy as adding it
to `/init.d` folder inside initramfs with correct name as script run order is
determined alphabetically. That's why scripts filenames are in `00-<name>`
format. I decided to add `07-encrypt_decrypt` script which should run before
anything else requiring filesystem access. To encrypt partition without losing
data we have to:

- Lower filesystem size so our new LUKS header can fit inside partition
...
- Decrypt partition, check and expand filesystem so we use rest of available
  space on partition
...
We are planning small changes to this feature, that is moving TPM2 enrollment
to systemd service that starts when we boot with Secure Boot enabled.
Enrolling PCR 7 before enabling and/or provisioning SB would result in needing
to update those PCRs in LUKS header anyway. Another change related to that will
be allowing user to define encryption password as recovery password generated by
`systemd-cryptenroll` isn't easy to write without mistakes. Currently, password
is generated in below format:

```text
xxxxxxxx-xxxxxxxx-xxxxxxxx-xxxxxxxx-xxxxxxxx-xxxxxxxx-xxxxxxxx-xxxxxxxx
```

Implemented script also allows us to encrypt/decrypt multiple partitions
depending on command-line arguments passed to kernel. This option is used with
next feature, [A/B update](#ab-update), which also adds `rwoverlay` partition
and a second `rootfs`, both of which need to be encrypted.

## A/B update

A/B update allows us to do secure updates (secure in the way that we won't brick
device which is being updated).
It works by updating inactive partitions, B in case we are using A, or A if we
are using B. After update we can boot into newly updated partition. If update
failed and system can't boot then on the next boot we will boot into non-updated
partition
...
If we print disk/image layout it would look like this (with encryption
feature):
...
we would update only singular files (e.g. kernel A or B) or configurations with
more slots than just A and B.

In the beginning both slots are nearly identical, with only changes between
partitions are:

- On boot partition the only difference is `.cmdline` section in UKI file to
  make sure it mounts correct `rootfs`
- On rootfs partition the only change is in `/etc/fstab` to make sure systemd
  mounts correct `boot` partition

There is also fifth partition, `rwoverlay` which is used as `overlayfs` which is
mounted on `/`. This is needed so we can keep configuration changes between
updates/slots, e.g. to keep user password we changed on first boot. This
partition is created on first boot and uses all remaining disk space.
...
- Add `Zarhus A` and `Zarhus B` with `efibootmgr`. Set `Zarhus A` as
  default/first booting option and `Zarhus B` as second

Normal/next boot:

- Boot from default partition, `Zarhus A` or `Zarhus B`, which depends on
  `BootOrder` or `BootNext`
- After booting into OS confirm that OS booted correctly with `otab confirm`.
  This does nothing unless update is in progress.

Update:

- Set variables which are used to check if update is in progress. We must set
  those in way that'll persist between reboots. Currently, we are using EFI
  NVRAM but as we are not using those variables outside of OS (e.g. in
  initramfs) then we could've done it via file somewhere on `rootfs`
  (`overlayfs`) partition
- Update inactive partitions
- Set `BootNext` to inactive boot partition
- Reboot

After that continue with `Normal boot` steps. We should boot from slot defined
in `BootNext`. If for some reason system doesn't manage to boot then on the next
one we will boot based on `BootOrder` which didn't change. If we manage to boot
into OS then `otab confirm` will detect update in progress and swap `Zarhus A`
and `Zarhus B` `BootOrder` to make updated slot the default one.

### SWUpdate

Under the hood we are using `SWUpdate` to update our system. To use it we need
to build cpio archive in correct format:
...
`sw-desctiption` is configuration file used by SWUpdate to know what to update
and how. In case of Zarhus OS it contains:
...
Our `sw-description` contains 2 slots, A and B with defined:
...
`.direct.p<x>.gz` files in cpio archive are partition images:
...
They are generated by `do_image` task and deployed by separate task to allow
them to be used by `zarhus-swu-image` target which generates `.swu` file.
Deployment boils down to compressing `.direct.p<x>` images and copying them to
`${DEPLOY_DIR_IMAGE}` folder.

### Building swu image

Building swu image is fairly easy thanks to
[swupdate.bbclass](https://github.com/sbabic/meta-swupdate/blob/master/classes-recipe/swupdate.bbclass),
just add files needed to be included in `.swu` file to `SWUPDATE_IMAGES` and
add all dependencies (e.g. base image) to `IMAGE_DEPENDS` to build it before swu
image.

### A/B partition differences

Zarhus A and B partitions differ in small way. On boot partitions UKI files have
...
Creating second UKI was done by copying first one and replacing its `.cmdline`
section with modified one. It can be done by `objcopy --update-section`:
...
This plugin decides which files to use for prepared partition based on 3
...
Empty variable is treated as if it wasn't defined.

---

Modifying `/etc/fstab` wasn't as straightforward to implement, at least I
haven't found easier way. I choose to write [custom Wic
plugin](https://github.com/zarhus/meta-zarhus/blob/develop/meta-zarhus-distro/scripts/lib/wic/plugins/source/rootfs-sed.py)
which applies `sed` filter on `/etc/fstab` during partition preparation.
To make it easier for myself and to not duplicate too much code I inherited from
...
Variable name containing `sed` filter can be passed via
`--sourceparams "sed_filter_var=<VARIABLE>"` argument in `wks` file.
After modifying `fstab` we need to change couple variables to inform base class
to use our new, modified copy of `fstab`:
...
Encrypting/decrypting all 3 partitions (decrypting inactive partition is needed
to allow updates) was done by modifying `encrypt_decrypt` initramfs script to
allow for specifying multiple partitions as first version worked only with
`rootfs`.
I implemented one difference in how `rwoverlay` is encrypted. As in-place
encryption without losing data can take a while, depending on partition size,
and `rwoverlay` is created on first boot I decided to use
`cryptsetup luksFormat` command instead of `cryptsetup reencrypt`:

```sh
cryptsetup luksFormat --type luks2 --key-file "temp_key" "/dev/disk/by-partlabel/rwoverlay"
```

It is much faster, whether partition has 50 GB or 1 TB. Creating encrypted
partition this way results in needing to recreate filesystem, so I also had to
add another script, `create_overlay_fs` which runs after `encrypt_decrypt`.

I also needed to modify deployed `rootfs` partitions used by `.swu` file as
during encryption we are decreasing filesystem size to make space for LUKS
header which results in update failing with information that there is not
enough space on device. I used mostly identical commands that are used during
encryption:
...
The problem was that to close encrypted partition I first had to `umount` it (as
it was busy otherwise), and I couldn't `umount` it because it was in use.
To fix that I had to go back to initramfs during shutdown and add shutdown hooks
that `umount` and close everything. I found a way to go back to initramfs by
researching how `dracut` does it. [The initrd Interface of
...
It's easy to check whether this work, just do those steps manually and create
`shutdown` shell script which starts `sh`. If you enable verbose logging you
should see:
...
and in shell you can check rootfs mount point with `df -h /`
...
To add shutdown script I decided to copy and minimally modify
...
- Remove filesystem mounting e.g. `/dev` as after `pivot_root` those
  mount points still exist.
...
- Add `export WATCHDOG_USEC=60000000` - it's set to 0 during shutdown which
  ...
  I choose value that's seen during reboot.

- Handle failure - In case of failure I immediately force requested action
  (shutdown, halt, reboot or kexec), skipping rest of hooks

After that, adding shutdown hooks is done the same way it is done for init
hooks. I created 3 hooks, called in described order:
...
- `finish` - last script, does action requested by user and passed to `shutdown`
  script by systemd
...
It wasn't easy implementing all those separate functionalities and making sure
they work separately and together, but I think it's mostly done. Let's test
...
- Encrypt partitions. After first encryption we are asked to save recovery key
  ...
  After confirming, enroll TPM and wipe temporary key:
  ...
  And encrypt rest of partitions. `rwoverlay`:
...
One failure is due to running `debug` image which allows root account.

---

To test update I copied `.swu` image to running system via `scp` and started
update with `otab update <file>`
...
After update, we can check that we booted from correct partitions:
...
There are multiple possibilities, but next step would be to change encryption
workflow a little:

- encrypt with user supplied password
- enroll TPM2 if system booted with SB enabled

Current update implementation could probably be also improved, e.g. by creating
installer or expanding first boot scripts we could create slot B during runtime
which would result in much smaller images (half the size without compression).
It should also be possible to use only single partition image in `.swu` update
image and either keep any files that differ between slots as separate files in
`.swu` archive or modify them in `otab-shell` script after inactive partition is
updated.

I'm also nearly finished with release CI for `meta-zarhus` which will allow us to
create releases for multiple platforms with different features. I hope we
will expand this CI in the future to include public cache upload to lower build
time and possibly add more sophisticated tests like [DTS E2E
tests](https://github.com/Dasharo/open-source-firmware-validation/blob/develop/dts/dts-e2e.robot).
Those would be very helpful in detecting any problems or regressions. This is
especially important as we have multiple features, and it is hard to test every
feature and platform combination manually.

If anyone is interested then I invite you to
[Zarhus Developers
Meetup](https://events.dasharo.com/event/6/zarhus-developers-meetup-2) where
I'll also present what I've written here in more abbreviated form.
