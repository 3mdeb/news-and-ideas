...
All actions will be performed on Ubuntu 18.04.
...
a package called meta-raspberry pi that we need to add to our working directory
...
This will tells Yocto to use `sdimage-raspberrypi.wks` file when creating a disk image. And the
second line which is responsible for creating two files. Compressed disk image `wic.gz` and
`wic.bmap` is a block map file.
...
Then press enter and log as root.
